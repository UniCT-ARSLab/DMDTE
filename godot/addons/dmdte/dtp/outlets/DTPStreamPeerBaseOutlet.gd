class_name DTPStreanPeerBaseOutlet extends DTPOutlet

var socket_server: SocketServer
var conns: Array[StreamPeerUDS] = []
...
func _take_connections():
	while socket_server.is_listening():
		var conn:StreamPeer= socket_server.take_connection()
		if not conn: break
